...
(defmethod %ring-apply-hash-function ((ring ring) value)
  (with-slots (hash-function) ring
    (funcall hash-function value)))

(defmethod %ring-hash-virtual-node ((ring ring) node-hostname index)
  (%ring-apply-hash-function ring (format nil *virtual-node-key-format-string* node-hostname index)))

(defmacro %for-each-virtual-node-with-finder (ring node-hostname &body body)
  `(with-slots (replication-factor finder) ,ring
...
(defmethod ring-insert ((ring ring) node-hostname)
  ...
  (%for-each-virtual-node-with-finder ring node-hostname
...
(defmethod ring-find-virtual-node-for ((ring ring) value)
  (let* ((hashed-value (%ring-apply-hash-function ring value))
